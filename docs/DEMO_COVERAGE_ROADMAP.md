...
1. **保持当前 PR 主线**：FastWAM evidence chain + 规划重构，不新开 PR。
2. **补 R0/R1 任务规格**：`kitchen_counter_sorting_v1` 与 `drawer_pick_place_v1` 已进入 R1；下一步补 `laundry_sorting_v1` 或 `trash_sorting_v1`。
3. **抽象通用 mock primitives**：object-in-region、container、drawer state、category routing、stage predicates。
4. **做 demo pack**：`make demo-extended` 已可一条命令跑 `tabletop + towel + kitchen + drawer`；下一步再补统一 summary。
5. **推进 R3 replay/offline action**：先接 LeRobot/BridgeData 小样本，验证 observation/action 对齐。
