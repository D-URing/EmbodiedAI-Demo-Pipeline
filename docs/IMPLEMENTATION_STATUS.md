...
| 快速入口 | `embodied-demo run --config configs/runs/tabletop_sorting_mock.yaml` / `embodied-demo run --config configs/runs/tabletop_sorting_mock.yaml` | `demo` 运行两个 MVP；`demo-extended` 运行四个 R1 household mock demo |
...
| Slurm 样例 | `experiments/lerobot/pusht_act_smoke/launch.sh` | 给未知集群一个可改的提交模板 |
| 快速入口 | `bash experiments/lerobot/pusht_act_smoke/launch.sh` | 在已安装 LeRobot 的 CUDA 节点上启动真实训练 smoke |
...
- `bash experiments/lerobot/pusht_act_smoke/launch.sh` 必须在有 CUDA 的 LeRobot 环境中运行；没有 GPU 会失败。
...
| Slurm 样例 | `experiments/custom/fastwam_realrobot_smoke/slurm.sbatch` | 给未知 NVIDIA 集群一个可改模板 |
...
FastWAM 现在有双路径定位：LeRobot-native FastWAM 是 LeRobot-first 主线上的优先 policy path；`D-URing/fastwam-realrobot-pipeline` 是 custom FastWAM / realrobot overlay，用于私有数据、内部 recipe 和未来自建模型。第一阶段不丢掉 overlay，但主线从“独立 FastWAM evidence”调整为“LeRobot dataset → policy train/load → offline inference → report”。

### 边界

- 本仓库不 vendor FastWAM 或私有 overlay 代码，不存权重、数据和 runs。
- `bash experiments/custom/fastwam_realrobot_smoke/launch.sh` 必须在已准备好的 FastWAM CUDA 环境中运行；没有 GPU 会失败。
...
3. 实现 LeRobot dataset inspection 和 inference experiment，补齐 LeRobot data-to-inference 主线。
4. 在 NVIDIA 集群运行 FastWAM `pilot`，把 overlay 真实日志作为 custom backend 证据交给 `embodied-demo report-fastwam`。
...
## 2026-07-14：LeRobot-First Reframe

状态：已接受 ADR-0003，项目主线调整为 LeRobot-first。
...
| 模型 registry | `references/model_registry.yaml` | 记录 ACT/PushT、LeRobot-native FastWAM、custom FastWAM overlay 的状态与存放约定 |
| artifact 规范 | `docs/MODEL_ARTIFACTS.md` | 说明模型、dataset、checkpoint 下载指引和存放位置；明确大文件不入库 |
...
| inference smoke | `scripts/lerobot/run_policy_inference_smoke.py`、`scripts/lerobot/run_inference_smoke.sh`、`bash experiments/lerobot/diffusion_pusht_infer/launch.sh` | 加载本地 checkpoint/pretrained policy，对 dataset sample 输出 action evidence |
| LeRobot chain report | `scripts/lerobot/generate_data_to_inference_report.py`、`python scripts/lerobot/generate_data_to_inference_report.py` | 汇总 dataset/inference/training 证据生成 report/handoff |
