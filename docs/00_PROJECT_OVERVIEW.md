...
当前第一条 LeRobot-native demo 是 **ACT on PushT**，默认配置为 `dataset.repo_id=lerobot/pusht`、`policy.type=act`。已封装 `make lerobot-data-smoke`、`bash experiments/lerobot/pusht_act_smoke/launch.sh` 和 `bash experiments/lerobot/diffusion_pusht_infer/launch.sh`；默认不下载大文件。
...
2. 实现 `bash experiments/lerobot/diffusion_pusht_infer/launch.sh`，证明 policy 加载和 action 输出；
